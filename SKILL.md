...
当前版本：3.6.0。详情见 [CHANGELOG.md](CHANGELOG.md)。Grok Build / Antigravity / Pi 宿主映射见 [harness-grok-antigravity.md](references/harness-grok-antigravity.md)。
...
- **因果脊柱**：主要节点用“因为 / 所以 / 但”连接，不靠“然后”续命。
- **主题保持为问题**：让不同人物以行动回答同一问题，不替读者总结标准答案。
- **风格来自观察**：用视角、细节、句法和意象建立声音，不靠形容词堆砌。
- **场景必须改变压力**：可用悬念、决定、情绪落点或余韵结束，不强制章章反转。
- **写作与审稿分离**：先完整写出一个单元，再派出隔离盲读；禁止同上下文填写盲读。
- **脚本只是烟雾报警器**：字数和关键词统计只能提示复核位置，不能证明文学质量。
- **拒绝伪完成**：大纲不算正文，口头进度不算落盘，检查通过不等于作品成熟。

## 写作模式

默认**作者模式**。只有用户明确说“自动写完整本”“批量写”“快写”时进入**工厂模式**。

| | 作者模式 | 工厂模式 |
|---|---|---|
| 冷启动 | 宪章 + `05-声音锁.md` + POV 声音样本 + 上一章全文 | 有界冷启动：摘要 + 宪章 + 上一章末尾 800 字 |
| 感知包 | 每章完整 | 可缩短 |
| 盲读 | 每章硬门，必须隔离调用 | 每 5 章抽检；未抽检须标注“未隔离” |
| 字数 | 报告 + 软上限；写到压力变化完成即停 | 可启用下限（长篇 3000 / 快写 2500 / 短篇 6000） |
| 失败 | 盲读复述不出则改稿 | 事务失败重试，连续 2 次停机留恢复点 |

习惯篇幅见项目设定，不是验收失败项。需要加长时只允许增加新的策略失败。
...
1. 定位当前模式：策划、连载、短故事、批量、修改、收尾、翻译、导出或诊断；并标明作者或工厂。
2. 查找已有文件；先读再写，不凭记忆覆盖用户正文。
3. 把可恢复的产物写入文件。正文与工作材料分开。
4. 完成当前最小闭环：策划交付可决定的方向；章节交付正文、隔离盲读、复盘和状态；修改交付真实改动；检查交付证据。
...
- 任务卡、感知包、沙盘、盲读、检查结果和修改记录只进入 `workspace/chapters/`。
...
- `05-声音锁.md`：[voice-lock-template.md](references/voice-lock-template.md)
- `99-进度仪表盘.md`：[progress-dashboard-template.md](references/progress-dashboard-template.md)（写作模式默认作者）
...
每章按以下顺序执行。工厂模式可压缩记录，不能跳过感知包、验收和状态回写。

### 1. 有界冷启动

**作者模式**只读：`99-进度仪表盘.md`（滚动前情摘要只作情节索引）、`00-大纲.md` 创作宪章区、`05-声音锁.md`、本章 POV 人物卡中的声音样本与“首先会注意的细节”、**上一章全文**（第 1 章无此条）。声音锁优先级高于场景计划。

**工厂模式**只读：仪表盘滚动前情摘要、宪章、POV 人物卡、上一章末尾 800 字。

冲突时以正文为准并修复仪表盘。

### 2. 感知包

写入 `perception.md`，见工作台模板。正文只允许从感知包进入，不允许从“本章要揭示线索”进入。缩短版至少保留第一眼、误判、不肯说的半句。人物出场时执行 [14-角色沙盘模式.md](references/14-角色沙盘模式.md)；深度发言的产物就是感知包。

任务卡仍写存在理由、不可逆变化和代价，不能替代感知包。

### 3. 设计场景（可选）

标准模式可在 `scene-plan.md` 写因果备忘。优先写成“因为……所以……但……”。正文以感知包为准。首章读 [opening-design.md](references/opening-design.md)，对白读 [dialogue-writing.md](references/dialogue-writing.md)，场景失效读 [scene-design-v2.md](references/scene-design-v2.md)，结尾读 [ending-design.md](references/ending-design.md)。

### 4. 写正文

写入 `manuscript/zh/第XXX章-标题.md`。稳定 POV；用少量有判断力的细节；对白让人物争取东西并保护不愿说的东西；让关键意象自然复现。写到压力变化完成即停。

### 5. 派出盲读

按 [blind-read.md](references/blind-read.md) 派出**隔离** subagent 或新会话，只给本章正文（非首章加上一章全文）。禁止打开大纲、档案、仪表盘和工作台。禁止同上下文填写盲读。Grok 用 `spawn_subagent`（优先 `blind-reader`）；Antigravity 用 `invoke_subagent`（优先 `blind-reader`）；Pi 用 `pi -p --no-context-files --no-skills`。细则见 [harness-grok-antigravity.md](references/harness-grok-antigravity.md)。无法隔离则停止交付，请用户另开只读正文的对话。

盲读复述不出不可逆变化，必须改正文后重新派出，不得只改复盘。处理走神、复述失败和作者腔后，把问题句与好句回写 `05-声音锁.md`。

### 6. 复盘与机械检查

盲读通过后再写 `review.md`，必须含「盲读指出 / 正文改动」。同轮自我边注不能代替盲读，也不是完成步骤。
...
工厂模式验收加 `--factory` 或 `--fast`。逐条查看命中原句：正确表达可以保留。禁止为降低“然而 / 因为 / 四字”计数而改文。把摘要、选择、代价、状态写入复盘与仪表盘滚动前情摘要。验收失败则补写后重跑。
...
4. 写到压力变化与收束完成即停；6000 字是习惯参考，不是作者模式验收失败项。不得在对话中直接输出完整正文；除非用户明确要求聊天全文。
5. 按 [blind-read.md](references/blind-read.md) 派出隔离盲读，再运行 `python3 scripts/check_short_story.py <短故事文件路径>`，人工复核高潮是否由人物选择触发、主线是否闭合、结尾是否改变了开头意象。
...
“自动写完整本”“全部写完”“autopilot”等明确请求才触发。这是**工厂模式**。若用户要作者质量，不要开自动驾驶。

- 从用户素材推断非核心缺口；核心方向采用故事发现中的推荐项。
- 以“完成一章正文 + 工作台 + 状态回写 + 事务验收”为原子事务；当前轮可继续时进入下一章。
- 每章完成后必须运行 `python3 scripts/check_chapter_transaction.py <小说目录> --factory <章节号>`。失败则补写并重试一次；连续 2 次失败停机，写入恢复点，不得假装通过。
- 每 10 章或卷末：作者模式改为隔离盲读连读最近 3 章，只问是否还认得这些人；工厂模式可写 `workspace/audits/卷X-审计.md` 作附录。
- 同批多书完本后运行 `python3 scripts/check_cross_book_similarity.py novels`。
- 不用套路表自动填充核心设计。文件写入失败、上下文无法可靠恢复或用户打断时暂停。

## 批量生产

用户要求“批量写”“自动写多本”或一次给出多个 premise 时，读取 [batch-production.md](references/batch-production.md)。批量使用工厂完成定义，不与作者模式共用质量故事。
...
同一协议可用于会话内连写、外部 CLI 循环或多实例并行。Grok 用 `/workflow chinese-novelist-factory` 或 `grok -p`；Antigravity 用 `/factory-chapter`、`/goal` 或 `agy -p`；Pi 用 `/factory-chapter` 或 `pi -p`。每个原子事务仍须验收。
...
6. 重读改后文本，需要时重新派出盲读，运行相关脚本，更新 `revision-notes.md` 和受影响状态。
7. 汇报实际改了什么；不要只给建议却不修改文件。

## 收尾期

按 [editorial-revision.md](references/editorial-revision.md) 分轮处理：读者通读改为隔离盲读；结构；人物；场景；语言；结局；连续性。
...
- 人物深度与声音：[character-template-v2.md](references/character-template-v2.md)、[character-building.md](references/character-building.md)、[voice-lock-template.md](references/voice-lock-template.md)
...
- Grok Build / Antigravity / Pi 宿主：[harness-grok-antigravity.md](references/harness-grok-antigravity.md)
...
| 字数硬下限 | `python3 scripts/check_chapter_wordcount.py <文件> --strict-min 3000` |
| 章节事务验收 | `python3 scripts/check_chapter_transaction.py <小说目录> <章节号>` |
| 工厂事务验收 | `python3 scripts/check_chapter_transaction.py <小说目录> --factory <章节号>` |
