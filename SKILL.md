...
- **用户指令矛盾**（如"写轻松点"又说"要高潮"）：优先执行具体指令，暂停确认模糊意图；自动驾驶模式下以最新指令为准
- **项目不存在**（用户说"继续写"但无小说目录）：引导进入策划期，创建新项目；自动驾驶模式下直接启动新项目策划流程
- **进度仪表盘缺失**：自动重建，从已有章节文件中提取状态信息补全
- **章节文件损坏/缺失**：回退到上一章状态，提醒用户检查，不自行假设内容；自动驾驶模式下跳过该章并标注
- **字数不达标时**：标注具体差距（如"当前2400字，差600字"），给出扩写建议而非直接补注水内容；自动驾驶模式下自动尝试扩写一次
- **用户长期中断后返回**：先读取进度仪表盘和最近2章摘要，简报当前状态后确认方向
- **脚本执行失败**：跳过脚本检查，改用人工对照 quality-checklist.md 逐项自检，在交付时注明"脚本未运行"
- **人物设定冲突**：以最新文件为准，在交付时列出冲突点提醒用户确认；自动驾驶模式下以大纲设定为权威来源
...
**自动驾驶模式**：不追问，从用户初始请求中最大程度推断，推断不出的使用题材模板自动填充（详见「自动驾驶模式」章节）。
