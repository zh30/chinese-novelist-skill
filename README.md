...
| 有限信息 | 角色只看到自己应该知道的事实 |
| 角色发言 | 感知包：第一眼、误判、不肯说的半句、保护策略 |
| 导演裁决 | 按人物一致性、因果、主线、代价排序 |
| 回写状态 | 更新角色文件、沙盘记录和进度仪表盘 |

使用原则：

- 复杂冲突展开完整沙盘；普通章节也必须有缩短版感知包，不能跳过。
- 一个关键角色一个运行时文件。
- 角色可以反抗大纲，但最终由导演裁决。
- 角色不写正文，只提供感知包；简单章也必须有缩短版感知包。
...
| [ending-design.md](references/ending-design.md) | 结局与余震 |
