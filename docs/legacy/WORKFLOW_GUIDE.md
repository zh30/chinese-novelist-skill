# 工作流可视化指南

> **已归档。** 以 `SKILL.md` v3.4+ 和 [plans/2026-09-11-写作水准优化执行方案.md](../plans/2026-09-11-写作水准优化执行方案.md) 为准。

> 一张图看懂 chinese-novelist-skill 的完整工作流

---

## 整体流程图

默认流程是长篇小说 / 网文三阶段。用户明确要求“短故事 / 短篇故事 / 写一篇完整故事”时，走下方短故事旁路，不进入连载期。

```
┌────────────────────────────────────────────────────────────────────┐
│                         三阶段工作流                                 │
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│   ┌──────────────┐      ┌──────────────┐      ┌──────────────┐    │
│   │   策划期     │ ───→ │   连载期     │ ───→ │   收尾期     │    │
│   └──────────────┘      └──────────────┘      └──────────────┘    │
│        │                      │                      │            │
│        ▼                      ▼                      ▼            │
│   极简大纲(10字段)          按章推进                  完稿检查        │
│   首章任务卡                自动维护悬念            伏笔回收        │
│   进度仪表盘                质量检查                结局收束        │
│                                                                    │
│   出口条件：              触发词：                触发词：          │
│   首章任务卡完成          "继续写"              "写完了"           │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

### 短故事旁路

```
用户说："写一篇短故事 / 短篇故事 / 完整故事"
         │
         ▼
┌─────────────────────────────┐
│ 1. 短故事任务卡              │
│    premise / 主角 / 冲突 / 结局 │
└─────────────────────────────┘
         │
         ▼
┌─────────────────────────────┐
│ 2. 完整剧情骨架              │
│    开端→升级→反转→高潮→收束 │
└─────────────────────────────┘
         │
         ▼
┌─────────────────────────────┐
│ 3. 写完整正文                │
│    ≥6000字，可分段或换行     │
└─────────────────────────────┘
         │
         ▼
┌─────────────────────────────┐
│ 4. 完稿复盘                  │
│    字数 / 主角变化 / 结尾闭合 │
└─────────────────────────────┘
```

---

## 详细流程图

### 阶段 1：策划期（第 1 次使用）

```
用户说："帮我写本悬疑小说"
         │
         ▼
┌─────────────────────────────┐
│ 1. 引导填写极简大纲          │
│    (10个问题，5分钟)         │
│                              │
│    问题示例：                │
│    - 主角是谁，想要什么？    │
│    - 核心冲突是什么？        │
│    - 结局是什么？            │
└─────────────────────────────┘
         │
         ▼
┌─────────────────────────────┐
│ 2. 生成基础文件              │
│                              │
│    novels/我的小说/           │
│    ├── 00-大纲.md            │
│    └── 99-进度仪表盘.md      │
└─────────────────────────────┘
         │
         ▼
┌─────────────────────────────┐
│ 3. 生成书名候选(3-5个)       │
│    用户选择确认              │
└─────────────────────────────┘
         │
         ▼
┌─────────────────────────────┐
│ 4. 进入连载期                │
│    开始写第1章               │
└─────────────────────────────┘
```

---

### 阶段 2：连载期（日常使用）

```
用户说："继续写"
         │
         ▼
┌──────────────────────────────────────┐
│ AI自动执行（< 2秒）                  │
│                                      │
│ 1. 读取 99-进度仪表盘.md              │
│    - 当前章节：第X章                 │
│    - 本章状态：[场景拆分完成]        │
│    - 活跃悬念：3个                   │
│                                      │
│ 2. 检查悬念状态                      │
│    - 001：🟢 活跃（第8章提及）       │
│    - 002：🔴 已过期（需立即处理）    │
│    - 003：🟡 即将过期                │
│                                      │
│ 3. 提取人物状态                      │
│    - 主角：[位置] [情绪] [目标]      │
│    - 核心价值观：[用于行为约束]      │
│    - 声音锚点：[3句标志性对白]       │
│                                      │
│ 4. 读取上一章结尾（最后500字）       │
└──────────────────────────────────────┘
         │
         ▼
┌──────────────────────────────────────┐
│ 规划本章                             │
│                                      │
│ 悬念操作：                           │
│ - 002（已过期）：提及               │
│ - 001（活跃）：推进（给出新线索）    │
│                                      │
│ 钩子强度：3级（与章节位置匹配）      │
│ 钩子类型：神秘物品                   │
└──────────────────────────────────────┘
         │
         ▼
┌──────────────────────────────────────┐
│ 写作执行                             │
│                                      │
│ 模式选择：                           │
│ - 用户说"快写" → 快速模式           │
│ - 默认 → 标准模式（场景拆分）        │
│                                      │
│ 标准模式步骤：                       │
│ 1. 场景拆分（3-6个场景）             │
│ 2. 写正文                            │
│ 3. 自检（字数+基础钩子）             │
└──────────────────────────────────────┘
         │
         ▼
┌──────────────────────────────────────┐
│ 交付章节                             │
│                                      │
│ 生成正文：manuscript/zh/第XXX章.md   │
│ 生成工作台：workspace/chapters/...   │
│ 更新进度仪表盘                       │
│ 显示质量概况                         │
└──────────────────────────────────────┘
         │
         ▼
┌──────────────────────────────────────┐
│ 质量检查（可选）                     │
│                                      │
│ 运行：check_ai_style.py              │
│                                      │
│ 🔴 红灯项（必须过关）：              │
│    ✓ 变化原则 ✓ 字数 ✓ 钩子 ✓ 人物  │
│                                      │
│ 🟡 黄灯项（建议优化）：              │
│    ⚠️ AI味中度：空泛形容词超标       │
│                                      │
│ 修复 → 参考 ai-style-examples.md     │
└──────────────────────────────────────┘
         │
         ▼
    [循环：继续写下一章]
```

---

### 阶段 3：收尾期

```
用户说："写完了"
         │
         ▼
┌──────────────────────────────────────┐
│ 完稿检查流程                         │
│                                      │
│ 1. 绿灯质量检查                      │
│    - 伏笔回收状态                    │
│    - 整体节奏曲线                    │
│    - 人物弧线完整性                  │
│                                      │
│ 2. 悬念回收验证                      │
│    - 所有活跃悬念已回收              │
│    - 没有僵尸悬念（未提及）          │
│                                      │
│ 3. 结局收束                          │
│    - 主线悬念兑现                    │
│    - 人物弧线完成                    │
│    - 设定规则一致                    │
└──────────────────────────────────────┘
         │
         ▼
┌──────────────────────────────────────┐
│ 导出成书                             │
│                                      │
│ python scripts/generate_epub.py      │
│ novels/我的小说                      │
│                                      │
│ 输出：我的小说.epub                  │
└──────────────────────────────────────┘
```

---

## 质量检查流程图

### 红绿灯机制

```
┌─────────────────────────────────────────────┐
│           每章交付前质量检查                   │
├─────────────────────────────────────────────┤
│                                             │
│  🟥 红灯项（必须过关，否则不能交付）          │
│  ┌─────────────────────────────────────┐   │
│  │ □ 变化原则：本章有不能删除的变化     │   │
│  │ □ 字数达标：≥3000字                  │   │
│  │ □ 钩子存在：结尾有明确悬念            │   │
│  │ □ 人物一致：行为符合设定              │   │
│  │ □ 短故事：≥6000字且剧情闭合           │   │
│  └─────────────────────────────────────┘   │
│                                             │
│  🟨 黄灯项（建议优化，AI自动检测）           │
│  ┌─────────────────────────────────────┐   │
│  │ □ AI味程度（check_ai_style.py）      │   │
│  │ □ 对白自然度                         │   │
│  │ □ 展示vs讲述比例                     │   │
│  └─────────────────────────────────────┘   │
│                                             │
│  🟩 绿灯项（完稿时统一检查）                 │
│  ┌─────────────────────────────────────┐   │
│  │ □ 伏笔回收状态                       │   │
│  │ □ 整体节奏曲线                       │   │
│  │ □ 人物弧线完整性                     │   │
│  └─────────────────────────────────────┘   │
│                                             │
└─────────────────────────────────────────────┘
```

---

## 悬念管理流程图

### 悬念生命周期

```
┌──────────────────────────────────────────────┐
│              悬念生命周期状态机                 │
├──────────────────────────────────────────────┤
│                                              │
│  ⚪ 待激活 ──→ 🟢 活跃 ──→ ✅ 已回收        │
│              │    │                         │
│              │    ├─→ 🟡 即将过期           │
│              │    │      （5章未提及）        │
│              │    │                         │
│              │    └─→ 🔴 已过期             │
│              │           （10章未提及）       │
│              │                               │
│              └────────→ 🚫 已废弃            │
│                                              │
└──────────────────────────────────────────────┘

关键规则：
- 活跃悬念上限：6个
- 每章至少提及1个已有悬念
- 每3-5章推进至少1个悬念
```

### 悬念规划流程

```
每章写作前：
         │
         ▼
┌────────────────────────────┐
│ Step 1: 悬念盘点            │
│                             │
│ 查看进度仪表盘：             │
│ - 活跃悬念（待推进）         │
│ - 即将过期（需提及）         │
│ - 计划新增（检查上限）       │
└────────────────────────────┘
         │
         ▼
┌────────────────────────────┐
│ Step 2: 悬念分配            │
│                             │
│ 本章操作：                  │
│ - 悬念001：推进（新线索）    │
│ - 悬念002：提及（保温）      │
│ - 悬念003：不处理           │
└────────────────────────────┘
         │
         ▼
┌────────────────────────────┐
│ Step 3: 强度检查            │
│                             │
│ 本章位置：第15章（50%）      │
│ 要求强度：4级               │
│ 实际强度：4级（突然揭示）    │
│ 匹配度：✅                  │
└────────────────────────────┘
         │
         ▼
    [开始写作]
```

---

## 三层叙事节奏图

### 宏观层（全本）

```
紧张度
  5 │                               ★
    │                          ╱    ╲
  4 │             ★         ╱          ★
    │          ╱   ╲    ╱                  ╲
  3 │    ★  ╱       ★                         ★
    │  ╱   ╲    ╱    ╲                     ╱
  2 │       ★╱         ★                ╱
    │                   ★            ╱
  1 │                      ★    ╱
    └────────────────────────────────────→
      0%   10%   25%   50%   75%   90%  100%
      │      │      │      │      │      │
      │      │      │      │      │      └─ 结局
      │      │      │      │      └─ 高潮前
      │      │      │      └─ 高潮
      │      │      └─ 中点转折
      │      └─ 小高潮
      └─ 开篇钩子

四幕结构：
[第一幕 25%] [第二幕 50%] [第三幕 20%] [第四幕 5%]
```

### 中观层（章节组）

```
黄金三章模式：

第N章        第N+1章      第N+2章
[铺垫]  →  [升级]   →  [小高潮+钩子]
  │            │            │
建立情境    矛盾激化    情绪高点+新问题
紧张度：★★☆  →  ★★★   →  ★★★☆

每3章一个节奏单元
单元结束后必须推进主线
```

### 微观层（单章）

```
单章内部结构：

[0-20%]    [20-50%]    [50-80%]    [80-100%]
  ↓           ↓            ↓            ↓
进入冲突   矛盾激化    转折/升级    钩子落下
紧张度：    紧张度：     紧张度：     紧张度：
★★☆       ★★★        ★★★☆       ★★★★☆

每章至少3次情绪变化
字数：≥3000字
```

---

## AI 味检测流程

```
运行 check_ai_style.py
         │
         ▼
┌──────────────────────────────┐
│ 检测9种AI味症状               │
│                              │
│ 🔴 重度：                     │
│   - 空泛形容词堆砌           │
│   - 情绪标签句               │
│   - 视角混乱                 │
│                              │
│ 🟡 中度：                     │
│   - 四字成语堆砌             │
│   - 解释性连接词             │
│   - 过度书面化对白           │
│   - 信息倾倒                 │
│                              │
│ 🟢 轻度：                     │
│   - 时间转折词滥用           │
│   - 句式均匀                 │
└──────────────────────────────┘
         │
         ▼
┌──────────────────────────────┐
│ 输出报告                      │
│                              │
│ 示例：                        │
│ ❌ 空泛形容词: 5/千字        │
│    建议：用具体动作替代       │
│                              │
│    改写示例：                 │
│    原：他很悲伤               │
│    改：他盯着手机屏幕，       │
│        拇指在玻璃上滑了三次   │
└──────────────────────────────┘
         │
         ▼
    [对照 ai-style-examples.md 修改]
         │
         ▼
    [重新运行检测]
```

---

## 人机协作流程

### v2 简化版（默认）

```
┌─────────────────────────────────────────┐
│           快速续写模式（v2）               │
├─────────────────────────────────────────┤
│                                         │
│  用户："继续写"                          │
│     │                                   │
│     ▼                                   │
│  AI自动执行（<2秒）：                     │
│     - 读取进度仪表盘                     │
│     - 提取关键上下文                     │
│     - 自动场景拆分                       │
│     - 开始写作                          │
│     │                                   │
│     ▼                                   │
│  AI："已读取（第X章，3个活跃悬念，        │
│       人物在[位置]）。正在写第X章..."    │
│     │                                   │
│     ▼                                   │
│  [直接开始写作]                          │
│                                         │
└─────────────────────────────────────────┘

不再需要：
❌ 正式化上下文传递格式
❌ 每次确认
❌ 手动填写多个文件
```

### v1 正式版（可选）

```
仅在特殊情况下使用：
- 人类修改后交接给AI
- 多人协作
- 长时间中断后恢复
```

---

## 文件依赖关系图

```
 novels/我的小说/
 │
 ├── 00-大纲.md              ◄─── 极简大纲（用户填写）
 │                                ├─ 主角欲望/恐惧
 │                                ├─ 核心悬念
 │                                └─ 章节规划
 │
 ├── 99-进度仪表盘.md        ◄─── 自动维护（AI更新）
 │         ▲                      ├─ 当前章节
 │         │                      ├─ 悬念状态
 │         │                      ├─ 人物状态
 │         │                      └─ 质量概况
 │         │
 ├── 01-人物档案-v2.md        ◄─── 人物引擎（用户填写）
 │         │                      ├─ 声音锚点
 │         │                      ├─ 缺陷-失败映射
 │         │                      └─ 弧线里程碑
 │         │
 ├── 02-世界观与伏笔.md        ◄─── 世界观（可选）
 │         │
 ├── 03-悬念追踪表.md          ◄─── 悬念管理（AI维护）
 │         │
 ├── manuscript/
 │   └── zh/
 │       ├── 第001章-标题.md  ◄─── 干净章节正文
 │       └── 第002章-标题.md
 │
 ├── workspace/
 │   └── chapters/
 │       └── 第001章-标题/    ◄─── 写作过程材料
 │           ├─ task-card.md
 │           ├─ scene-plan.md
 │           ├─ sandbox.md
 │           ├─ review.md
 │           └─ revision-notes.md
 │
 │
 └── 我的小说.epub            ◄─── 导出成品

 AI读取顺序：
 1. 99-进度仪表盘.md（获取当前状态）
 2. 03-悬念追踪表.md（检查悬念状态）
 3. 01-人物档案-v2.md（提取人物约束）
 4. manuscript/zh/第X-1章.md（读取上一章结尾）
 5. workspace/chapters/第X章/（如已存在任务卡）
```

---

## 常见问题决策树

```
问题：我应该用快速模式还是标准模式？
         │
         ▼
    用户说"快写""初稿"？
         │
    ┌────┴────┐
    │           │
   是          否
    │           │
    ▼           ▼
快速模式    用户说"修改"？
(跳过场景    │
 拆分)       ┌┴┐
             │ │
            是  否
             │   │
             ▼   ▼
        修改模式  标准模式
        (修改)  (完整流程)


问题：AI写的我不满意，怎么办？
         │
         ▼
    具体问题是什么？
         │
    ┌────┼────┬────────┐
    │    │    │        │
对白  节奏  细节   整体
不自然 太慢  不够   不满意
    │    │    │        │
    ▼    ▼    ▼        ▼
参考   参考  增加   重写
对白   节奏  描写   整章
指南   模板        或
                    切换
                    模式
```

---

## 版本对比

```
┌──────────────────────────────────────────────────────────┐
│               v2.0 → v2.1 优化                            │
├──────────────────────────────────────────────────────────┤
│                                                          │
│  AI味检测：6种症状     →  9种症状+批量模式              │
│             遗漏3种          完整覆盖                    │
│                                                          │
│  脚本：3个工具          →  7个工具                      │
│        缺人物/时间线检查    新增character_tracker         │
│                             check_timeline              │
│                             check_novel_health合并      │
│                                                          │
│  收尾期：仅出版门控      →  三步流程（绿灯→润色→门控） │
│            缺少润色         完稿三轮润色+版本管理       │
│                                                          │
│  多线叙事：无            →  线切换决策+POV规则           │
│            只能单线          支持群像多线               │
│                                                          │
│  中断恢复：一行描述      →  完整恢复协议                │
│            不够用            续写简报+校验+断路器恢复   │
│                                                          │
│  v1模板：未标注废弃      →  全部标注+指向v2             │
│                                                          │
└──────────────────────────────────────────────────────────┘
```

---

保存这张图，写作时对照使用！
